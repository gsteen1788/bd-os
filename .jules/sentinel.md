...
## 2025-06-29 - Prevent DoS via unvalidated AI ingestion
**Vulnerability:** Batch ingestion script (`learningIngestion.ts`) directly inserts unvalidated strings generated by AI into the database, risking database bloat or Denial of Service (DoS) if AI produces abnormally large or malformed outputs. It also leaked potential PII via raw `console.error` calls upon processing failures.
**Learning:** External or AI-generated inputs in background/batch scripts must be treated with the same zero-trust model as direct user input in HTTP endpoints, including length and structure validation.
**Prevention:** Wrap array processing loops containing `validateInput` (using `MAX_TEXT_LENGTH`) in a `try/catch` to securely skip invalid individual items rather than crashing the batch process or accepting malicious data, and use the central `logger` instead of raw `console.error`.
