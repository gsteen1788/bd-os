...
## 2024-05-29 - [CSRF Vulnerability in OAuth Flow]
**Vulnerability:** The OAuth flow implemented in `src/services/authService.ts` used a hardcoded `state` parameter (`state=12345`), and the Rust backend (`start_auth_server`) was only parsing and returning the `code`. This lack of state validation left the authentication flow vulnerable to Cross-Site Request Forgery (CSRF).
**Learning:** Hardcoded state parameters completely bypass the security guarantee of the `state` mechanism in OAuth, which is intended to prevent CSRF attacks by ensuring the request originated from the legitimate client. The backend must also facilitate this by returning the full query string so the frontend can extract and validate the `state`.
**Prevention:** Always generate a secure, dynamic, and high-entropy `state` parameter before initiating an OAuth flow. Ensure the backend returns the full query string, and validate that the returned `state` matches the dynamically generated one before exchanging the code for a token.
