...
## 2026-06-11 - Denial of Service via Unvalidated AI Inputs
**Vulnerability:** The AI ingestion script (`src/utils/learningIngestion.ts`) processed outputs generated by the Gemini AI and directly inserted them into the database without validating string length.
**Learning:** External inputs, even when generated by an AI service responding to a controlled prompt, are fundamentally untrusted and can occasionally generate wildly large outputs or hallucinated endless loops, which can lead to application crashes (OOM) or database bloat/DoS.
**Prevention:** All inputs, regardless of origin, must be validated before persistence. Use `validateInput(item, "Label", MAX_TEXT_LENGTH)` wrapped in a `try/catch` on each item of an externally sourced list to gracefully reject maliciously or accidentally large data without halting the entire process.
