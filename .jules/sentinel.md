...
## 2026-06-28 - Missing Validation on AI-Generated Batch Data
**Vulnerability:** The AI learning ingestion process in `src/utils/learningIngestion.ts` processed batch outputs from Gemini using `JSON.parse` and inserted the parsed strings directly into the database without validation.
**Learning:** External data, even when generated by an "internal" prompt, must be treated as untrusted. Injecting massive strings or unexpected control characters from AI hallucinational outputs could cause a denial of service (DoS) in the database or UI.
**Prevention:** Always validate all string values parsed from external sources (e.g. AI JSON outputs) using `validateInput` and `MAX_TEXT_LENGTH` inside a `try/catch` block before database insertion, to filter out malformed or dangerously large responses without crashing the entire batch process.
