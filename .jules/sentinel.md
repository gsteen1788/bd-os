...
## 2024-07-01 - Prevent DoS via Database Bloat in AI Batch Processes
**Vulnerability:** Unvalidated, potentially massive AI-generated strings were being directly parsed and inserted into the database during batch processing (e.g., in `learningIngestion.ts`), leading to a potential Denial of Service (DoS) vulnerability via database bloat.
**Learning:** External or AI-generated inputs, even when requested as structured JSON arrays, can still return arbitrarily long strings or malicious control characters. Batch scripts lacking defensive validation for individual array elements could crash entirely or bloat the database when processing maliciously crafted individual entries.
**Prevention:** Always use `validateInput` (with an appropriate constraint like `MAX_TEXT_LENGTH`) within a `try/catch` block for each item when iterating through parsed AI outputs or loosely typed arrays. This ensures invalid items are skipped securely without halting the entire batch process or poisoning the database.
