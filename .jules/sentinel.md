...
## 2024-10-27 - [PII Leakage and DoS in Learning Ingestion]
**Vulnerability:** Raw `console.log` and `console.error` calls were used in `src/utils/learningIngestion.ts`, which could leak PII or sensitive data generated by the AI to standard output. Additionally, the AI-generated array outputs (from `JSON.parse`) were inserted into the SQLite database without any input validation or length limits, exposing the system to database bloat or Denial of Service (DoS) attacks via maliciously crafted or extremely long text.
**Learning:** External or AI-generated inputs must be treated with the same suspicion as direct user inputs. Logging such data using standard `console` utilities risks unintended data exposure.
**Prevention:** Always use the centralized `logger` utility (`src/infrastructure/logger.ts`) which strips complex objects in production. Additionally, any externally generated data, especially array items from parsed JSON, must be passed through `validateInput` within a `try/catch` block before being persisted in the database, ensuring length limits (e.g., `MAX_TEXT_LENGTH`) are strictly enforced.
