...
## $(date +%Y-%m-%d) - Denial of Service and DB Bloat via Unvalidated AI Outputs
**Vulnerability:** The batch ingestion process (`learningIngestion.ts`) blindly inserted external strings generated by an LLM (Gemini) directly into the database array operations, creating a Denial of Service (DoS) and Database Bloat vector if the AI hallucinated excessively long outputs or invalid characters, crashing the entire batch process.
**Learning:** AI-generated outputs must be treated as untrusted user input. Using raw `console.log` for logs in processing loops can also leak sensitive details (if any existed in the file) or crash from overly verbose output, necessitating the use of `logger.ts`.
**Prevention:** Always wrap `validateInput` (which checks lengths and control chars) in a `try/catch` specifically for array/batch items, filtering and logging the skipping of invalid elements rather than letting them crash the entire `forEach` loop.
