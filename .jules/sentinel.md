...
## 2026-06-11 - Missing CSRF Validation in OAuth Flow
**Vulnerability:** The OAuth login flow in `src/services/authService.ts` used a hardcoded `state` parameter (`"12345"`) and the Tauri local web server `src-tauri/src/main.rs` stripped out query parameters when returning the authorization code. This made the application vulnerable to Cross-Site Request Forgery (CSRF) attacks, where an attacker could force a user into logging in with the attacker's account or trick the application into completing the flow initiated by the attacker.
**Learning:** Hardcoded state parameters completely defeat the purpose of the state variable in OAuth, which is to bind a specific login request to the browser session that initiated it. If the backend fails to pass the query string back to the frontend, this critical validation cannot occur.
**Prevention:** Always dynamically generate a high-entropy, random `state` parameter before redirecting to the authorization endpoint. Store it temporarily and ensure the returning callback (or local server) provides the full query string back to the application. Validate that the `state` returned from the authorization server strictly matches the locally generated `state` before exchanging the code for a token.
