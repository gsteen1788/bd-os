...
## 2024-05-29 - Secure Logger Implementation for PII Leakage
**Vulnerability:** Repositories and UI components were using raw `console.log` and `console.error`, which risked dumping full entity objects containing PII into production logs.
**Learning:** PII leakage often happens via generic error logging (e.g., `catch(e) { console.error("Error", e) }`) where the full error object or context entity is logged and output natively by the browser or Tauri process.
**Prevention:** Use the centralized secure `logger.ts` utility that suppresses non-error logs in production and actively strips complex error objects (falling back to `error.message` or `String(error)`) to prevent unintended exposure of raw entity data. Always avoid `console.*` in UI and repository code.
