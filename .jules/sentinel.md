...
## 2024-05-29 - Missing CSRF State Validation in OAuth Flow
**Vulnerability:** The OAuth flow implementation in `src/services/authService.ts` was using a hardcoded `state` parameter (`state=12345`) and completely ignoring it upon the redirect response from the authorization server. This made the authentication flow vulnerable to Cross-Site Request Forgery (CSRF) attacks, where an attacker could force a victim to log into an attacker-controlled account (login CSRF) or potentially leak authorization codes.
**Learning:** Hardcoding or ignoring the `state` parameter defeats its entire purpose in OAuth 2.0. The `state` parameter must be dynamically generated (using a CSPRNG), sent with the authorization request, and then strictly validated against the originally generated value when the callback is received to ensure the response corresponds to the request initiated by the legitimate user.
**Prevention:** Always generate a unique, cryptographically secure `state` parameter for every OAuth authorization request. Store it securely (or keep it in memory for native flows) and strictly validate that the `state` parameter returned in the redirect URL exactly matches the generated one before exchanging the authorization code for tokens.
