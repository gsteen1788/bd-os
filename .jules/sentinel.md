...
## 2024-05-29 - DoS via Unbounded AI-Generated Array Items
**Vulnerability:** The AI ingestion script (`learningIngestion.ts`) extracted learnings via Gemini as a JSON array. While the JSON array itself was parsed, the individual extracted string items within the array were blindly passed to a bulk database `INSERT` statement without any length checks or input validation via `validateInput`.
**Learning:** External AI models can generate unusually long, hallucinatory, or maliciously crafted (via prompt injection in the source document) outputs. Processing arrays of these outputs without validating the size of each item presents a significant Denial of Service (DoS) and potential injection risk to the database.
**Prevention:** To prevent DoS via database bloat or crashes, any AI-generated text or externally sourced string within arrays must be validated element-by-element using `validateInput` (with constraints like `MAX_TEXT_LENGTH`). Wrap this validation in a `try/catch` to gracefully skip invalid elements without crashing the entire batch ingestion.
