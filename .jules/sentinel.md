...
## 2025-02-14 - Unvalidated AI Output leading to Potential DoS
**Vulnerability:** The AI learning ingestion script parsed JSON outputs from Gemini and inserted them directly into the local SQLite database without size constraints.
**Learning:** External or AI-generated inputs can bypass typical application-layer validation if injected directly via backend utility scripts, risking database bloat and Denial of Service (DoS) if the AI generates excessively large arrays or strings.
**Prevention:** Always treat AI output as untrusted user input. Apply rigorous input validation (e.g., `validateInput` with `MAX_TEXT_LENGTH`) and wrap processing steps in `try/catch` to isolate and skip malformed elements during batch processing rather than crashing the batch or accepting dangerous data.
