...
## 2024-05-29 - Missing Input Validation on AI-Generated Content
**Vulnerability:** The AI-generated learnings in `src/utils/learningIngestion.ts` were inserted directly into the database without validation. While generated by an internal AI service (Gemini), AI models can hallucinate massive string outputs or output control characters, leading to DoS via DB exhaustion or memory bloat. Additionally, the file name parameter was passed unvalidated.
**Learning:** We cannot unconditionally trust data returning from external AI models. "Trust nothing, verify everything." If an AI prompt output exceeds schema constraints or length bounds, it can crash or DoS the application database just as effectively as a direct user exploit.
**Prevention:** Always validate external strings—even from AI models or local paths—using `validateInput` to strictly enforce maximum length bounds and sanitize out control characters before persisting them to the database.
