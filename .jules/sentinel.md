...
## 2024-05-18 - Missing CSRF State Validation in OAuth Flow
**Vulnerability:** The OAuth flow for Outlook Calendar connection in `src/services/authService.ts` used a hardcoded `state` parameter (`12345`) and the redirect server in `src-tauri/src/main.rs` failed to extract and return the `state` parameter to the client for validation, leaving the application vulnerable to Cross-Site Request Forgery (CSRF).
**Learning:** Hardcoding the `state` parameter in an OAuth authorization request defeats its purpose of binding the request to the specific user's session, enabling attackers to inject their own authorization code and potentially connect an attacker-controlled account to the victim's profile.
**Prevention:** Always generate a high-entropy, dynamic `state` parameter for every OAuth authorization request, parse the full query string returned to the redirect URL, and enforce that the returned `state` exactly matches the originally generated value before exchanging the code for a token.
