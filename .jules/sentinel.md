...
## 2024-05-29 - Stored XSS via URI Scheme Bypass in validateSafeUri
**Vulnerability:** The `validateSafeUri` function used a basic `trim()` before checking for dangerous schemes like `javascript:`. This allowed an attacker to bypass the validation by inserting newline or tab characters within the scheme name (e.g., `java\nscript:alert(1)`). When the browser eventually renders this URL, it ignores the whitespace and executes the JavaScript.
**Learning:** Checking for exact string matches or regexes against URI schemes is insufficient if the string contains invisible control characters or whitespaces that the browser ignores during execution. Standard `trim()` only removes leading and trailing spaces, not internal ones.
**Prevention:** Always aggressively strip whitespaces (`\s`) and non-printable control characters (`\x00-\x1F\x7F`) from URIs before performing security validations against dangerous schemes like `javascript:`.
