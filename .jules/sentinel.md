...
## 2024-08-16 - DoS via AI Generation Bloat in Data Ingestion
**Vulnerability:** The batch processing script `learningIngestion.ts` accepts raw strings directly from `generateContent` (AI outputs) via `JSON.parse` and blindly inserts them into the database (`INSERT INTO learnings`). Because `generateContent` leverages a loosely typed prompt and raw outputs, an attacker could manipulate the input image/file to cause the AI to generate an excessively large JSON string array, which would crash the application via Out-Of-Memory (OOM) or bloat the SQLite database, causing a Denial of Service.
**Learning:** `JSON.parse` does not strictly enforce schema or size bounds by default, and third-party AI APIs can generate unbounded responses when maliciously prompted via ingested files. We cannot blindly trust data simply because it comes from our "own" API wrapper if the underlying source data is untrusted.
**Prevention:** Ingestion and batch processing scripts that rely on parsed objects must iterate through items and explicitly pass them through `validateInput(..., MAX_TEXT_LENGTH)` to enforce length, type, and control character constraints before insertion. Wrap the item validation in a try/catch to ensure one malformed entry doesn't crash the entire batch process.
