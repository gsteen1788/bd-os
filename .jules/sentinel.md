...
## 2026-06-09 - Stored XSS via URI Scheme Whitespace Bypass
**Vulnerability:** The `validateSafeUri` function used a simple `.trim()` followed by regex matching (`/^javascript:/i`) to prevent dangerous URI schemes. This could be bypassed by inserting whitespace (e.g., `\n`, `\t`) or control characters inside the scheme (e.g., `java\nscript:alert(1)`). HTML parsers often ignore these characters in attributes like `href` or `src`, leading to Stored XSS.
**Learning:** Checking for dangerous URI schemes using regex requires first stripping all characters that the browser's URL parser might ignore. A simple `trim()` only removes leading/trailing whitespace, not embedded whitespace or control characters that can obfuscate the payload.
**Prevention:** Aggressively strip all whitespace (`\s`) and non-printable control characters (`\x00-\x1F\x7F`) from URI strings using `.replace(/[\s\x00-\x1F\x7F]+/g, '')` before testing them against regexes for dangerous schemes.
