...
## 2024-05-19 - Batching Repository Fetching
**Learning:** In React UI components, executing multiple independent asynchronous repository requests (e.g. `findAllSummaries`) inside `useEffect` causes multiple sequential state updates (`setState`). This staggered resolution leads to unnecessary component re-renders.
**Action:** Always wrap independent repository fetch promises within `Promise.all()` in the `useEffect` hooks. Resolving them together allows React to batch all the state update functions into a single render pass, reducing UI tearing and overhead.
