...
## 2024-05-26 - DaisyUI btn-ghost Class Dependence
**Learning:** In DaisyUI, using modifier classes like `btn-ghost` without the base `btn` class removes necessary structural styling (padding, alignment, hover states) leading to inconsistent or broken interaction targets.
**Action:** Always ensure UI elements using modifier button classes explicitly include the base `btn` class (e.g., `className="btn btn-ghost"`).
