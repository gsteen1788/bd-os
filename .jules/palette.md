...
## 2024-05-26 - Missing base btn classes for btn-ghost buttons
**Learning:** When fixing UI components using DaisyUI style classes, ensure all `btn-*` modifier classes (like `btn-ghost` or `btn-sm`) are preceded by the base `btn` class (e.g., `className="btn btn-ghost"`). Omitting `btn` causes missing base button styles, such as structural padding, proper height, and necessary touch targets.
**Action:** When adding or reviewing button styles using modifier classes like `btn-ghost`, always ensure the foundational `btn` class is also present.
