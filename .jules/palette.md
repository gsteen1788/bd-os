...
## 2024-05-24 - Missing base button classes for UI styling consistency
**Learning:** In DaisyUI/Tailwind setups, adding a modifier class like `btn-ghost` without the core base class `btn` breaks standard button styling, resulting in missing padding, hover states, and focus accessibility rings.
**Action:** Always ensure any button using a DaisyUI `btn-*` modifier also includes the base `btn` class (e.g., `className="btn btn-ghost"`).
