...
## 2024-11-23 - Fix Missing btn Class on Ghost Buttons
**Learning:** Using `btn-ghost` without the base `btn` class causes elements to miss critical structural styles, such as adequate touch targets (height/padding), border-radius, and base interactive states in the DaisyUI framework.
**Action:** Always ensure that utility modifier classes like `btn-ghost` or `btn-sm` are accompanied by their foundational base class (`btn`) when implementing new interactive elements.
