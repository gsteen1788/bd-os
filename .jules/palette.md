...
## 2024-05-25 - DaisyUI Base Classes Required for Modifiers
**Learning:** When using DaisyUI modifier classes like `btn-ghost`, `btn-sm`, or `btn-primary`, they only provide the color, variant, or size styles. Omitting the base `btn` class causes elements to lose structural padding, proper height, and necessary touch targets.
**Action:** Always ensure all `btn-*` modifier classes are preceded by the base `btn` class (e.g., `className="btn btn-ghost"`) to guarantee accessible touch targets and layout stability.
