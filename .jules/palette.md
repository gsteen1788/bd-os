...
## 2024-05-26 - Missing Base `btn` Class for Button Modifiers
**Learning:** In this project's custom CSS implementation mimicking DaisyUI, button modifier classes like `btn-ghost` do not automatically include base styles. Omitting the base `btn` class results in buttons missing structural padding, correct heights, and sufficient touch targets, negatively impacting usability and interaction accessibility.
**Action:** Always verify that any button modifier class (e.g., `btn-ghost`, `btn-sm`) is explicitly preceded by the base `btn` class in the `className` string.
