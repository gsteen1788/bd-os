...
## 2026-07-24 - Base btn class required for btn modifiers
**Learning:** When using DaisyUI button modifiers (like `btn-ghost`, `btn-sm`, `btn-outline`), the base `btn` class is strictly required. Omitting it leads to missing structural styles such as standard padding, correct heights, and accessible touch target sizes, causing inconsistent and hard-to-click UI elements.
**Action:** Always prepend `btn` when using button modifier classes (e.g., `className="btn btn-ghost"`). Never use a modifier standalone.
