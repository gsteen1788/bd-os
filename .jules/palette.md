...
## 2024-05-23 - Fixing missing base btn classes on MeetingPrep buttons
**Learning:** `btn-ghost` and other modifier classes require the base `btn` class to apply the standard button styling like structure and padding. Without it, buttons lose structural consistency and touch target sizes. This seems to be a common issue in DaisyUI-like custom CSS systems when applying multiple modifier classes.
**Action:** Ensure all `btn-*` modifiers are preceded by the base `btn` class (e.g., `className="btn btn-ghost"`).
