...
## $(date +%Y-%m-%d) - Fix missing base button class on ghost buttons
**Learning:** In utility-based CSS frameworks like DaisyUI, omitting the base `btn` class before modifier classes like `btn-ghost` leads to missing structural styles (padding, alignment, proper height). This results in a degraded user experience, such as missing touch targets and improper text alignment.
**Action:** Always ensure that utility modifier classes for buttons (e.g., `btn-ghost`, `btn-sm`) are preceded by the base `btn` class (e.g., `className="btn btn-ghost"`).
