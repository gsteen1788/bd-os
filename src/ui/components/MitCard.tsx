...
export const MitCard = memo(function MitCard({
    ...
    return (
        <div
            className={`card border border-[hsl(var(--color-border))] hover:border-primary/50 transition-all p-0 flex flex-col gap-0 group hover:shadow-lg hover:shadow-primary/5 cursor-pointer overflow-hidden relative focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2 ${viewMode === 'HISTORY' ? 'bg-base-300 opacity-80' : 'bg-base-200'}`}
        >
            {/* Card Header */}
            ...
            <div className="px-4 pb-4">
                <h3>
                    <button
                        className={`text-lg font-bold leading-tight text-left w-full group-hover:text-primary transition-colors focus:outline-none before:absolute before:inset-0 before:z-0 px-1 -ml-1 ${viewMode === 'HISTORY' ? 'line-through text-muted' : ''}`}
                        onClick={(e) => {
                            e.stopPropagation();
                            onEdit(mit);
                        }}
