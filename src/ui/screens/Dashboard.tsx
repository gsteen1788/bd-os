...
export function Dashboard() {
    ...
    // Optimization: Memoize grouped history to avoid re-calculating on every render
    const groupedHistory = useMemo(() => {
        if (viewMode === 'HISTORY') {
            return groupItemsByWeek(mits, 'updatedAt');
        }
        return {};
    }, [mits, viewMode]);
                ...
                <div className="flex-1 flex flex-col min-w-0">
                    <div className="grid-mit-cards p-6 overflow-y-auto flex-1 custom-scrollbar min-h-0">
                        {viewMode === 'HISTORY' ? (
                            ...
                            Object.keys(groupedHistory).map(weekLabel => (
                                <div key={weekLabel} className="col-span-full mb-2">
                                    ...
                                    <div className="grid-mit-cards">
                                        {groupedHistory[weekLabel].map(mit => (
                                            <MitCard
                                                key={mit.id}
                                                mit={mit}
                                                viewMode={viewMode}
