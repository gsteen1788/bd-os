...
export async function ingestLearnings() {
    ...
    for (const path in modules) {
        ...
        try {
            ...
            if (Array.isArray(parsedLearnings) && parsedLearnings.length > 0) {
                // Filter out invalid learnings to prevent crashing the batch
                const learnings = parsedLearnings.filter(learning => {
                    try {
