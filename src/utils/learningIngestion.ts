        ...
        try {
            ...
            if (Array.isArray(learnings) && learnings.length > 0) {
                ...
                for (const learning of learnings) {
                    try {
                        validateInput(learning, "AI Learning output", MAX_TEXT_LENGTH);
                        validLearnings.push(learning);
                    } catch (validationError) {
                        logger.warn(`Skipped invalid learning from ${fileName}: ${validationError}`);
                    }
                }
                ...
                if (validLearnings.length > 0) {
                    ...
                    logger.info(`Extracted ${validLearnings.length} learnings from ${fileName}.`);
                }
            }
