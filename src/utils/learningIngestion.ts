...
export async function ingestLearnings() {
    ...
    for (const path in modules) {
        ...
        try {
            ...
            let rawLearnings: string[] = [];
            try {
                rawLearnings = JSON.parse(jsonString) as string[];
            } catch (e) {
                console.error(`Failed to parse AI output for ${fileName}:`, e);
                continue;
            }
            ...
            if (Array.isArray(rawLearnings) && rawLearnings.length > 0) {
                ...
                if (validLearnings.length === 0) {
                    console.log(`No valid learnings extracted from ${fileName} after validation.`);
                    continue;
                }
