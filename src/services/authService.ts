...
class AuthService {
    ...
    async login(): Promise<void> {
        ...
        // Dynamically generate a secure state parameter to prevent CSRF attacks
        const stateParam = this.generateRandomString(32);

        const authUrl = `${AUTH_ENDPOINT}?client_id=${CLIENT_ID}&response_type=code&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_mode=query&scope=${encodeURIComponent(SCOPES)}&state=${encodeURIComponent(stateParam)}&code_challenge=${codeChallenge}&code_challenge_method=S256`;
