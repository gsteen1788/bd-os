...
class AuthService {
    ...
    async login(): Promise<void> {
        ...
        const stateParam = this.generateRandomString(32);

        const authUrl = `${AUTH_ENDPOINT}?client_id=${CLIENT_ID}&response_type=code&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_mode=query&scope=${encodeURIComponent(SCOPES)}&state=${stateParam}&code_challenge=${codeChallenge}&code_challenge_method=S256`;
        ...
        try {
            ...
            const queryParams: string = await invoke('start_auth_server', { port: 8400 });

            if (!queryParams) throw new Error("No authorization code received.");
            ...
            if (returnedState !== stateParam) throw new Error("CSRF validation failed: State mismatch");
